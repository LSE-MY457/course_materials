...
N = 1000
LATE = 2
cutoff = 5 # we won't use this just yet
...
```{r realized_data}
ggplot(data[data$X<cutoff,], aes(x = X, y = D)) + 
  geom_point(color = "darkblue", alpha = 0.5) + 
  geom_point(data = data[data$X >= cutoff,], aes(y = D), color = "darkred", alpha = 0.5) +
  geom_vline(xintercept = cutoff) + 
...
ggplot(data = data, aes(x = X, y = Y0)) + 
  ...
  geom_point(data = data[data$X<cutoff,], color = "darkblue", alpha = 0.5) + 
  geom_point(data = data[data$X >= cutoff,], aes(y = Y), color = "darkred", alpha = 0.5) +
  geom_vline(xintercept = cutoff) + 
  labs(title = "Realized Outcome", x = "X", y = "Outcome") +
  theme_minimal()
...
```{r rdrobust}
# Note that the default rdrobust analysis gives us the conventional point estimates and the robust bias-corrected SEs
...
ggplot(data = data[data$X<cutoff,], aes(x = X, y = Y0)) + 
  geom_smooth(color = "darkblue", method = "loess", alpha = 0.1, se = F) +
  geom_point(color = "darkblue", alpha = 0.5) + 
  geom_smooth(data = data[data$X >= cutoff,], aes(y = Y1), color = "darkred", method = "loess", alpha = 0.1, se = F) +
  geom_point(data = data[data$X >= cutoff,], aes(y = Y), color = "darkred", alpha = 0.5) +
  geom_vline(xintercept = cutoff) + 
  labs(title = "Realized Treatment Status", x = "X", y = "Treatment (D = 0,1)") +
  theme_minimal()
...
plac_df <- tibble(cutpoint = plac_c, est = plac_est, se = plac_se, placebo = "Placebo") %>% # create a dataframe of our placebo estimates
  rbind(tibble(cutpoint = cutoff, est = rd_est$coef[1], se = rd_est$se[3], placebo = "Actual"))%>% # add in the "real" estimate
  mutate(upper = est + 1.96*se,
         lower = est - 1.96*se)

ggplot(plac_df, aes(x = cutpoint, y = est, col = placebo)) +
  geom_point() +
...
fuzzy_data = tibble(
  ...
  X = runif(N, 0, 10),
  U = rnorm(N),
  # Next, assign a compliance score, which is the probability of being a complier (depending on U):
  c_score = plogis(-1 + 2*U),
  # Determine if a complier:
  complier = rbinom(N, 1, c_score),
  # Now generate a pscore for always- and never-takers:
  p_score = plogis(-1 - 0.25*X + 0.1*(X^2) + 2*U),
  ...
  compliance_type = case_when(
    complier == 1 ~ "complier",
    always_taker == 1 ~ "always_taker",
    TRUE ~ "never_taker"
  )
) %>%
  mutate(
    # Assign D0 and D1 based on compliance type:
    D0 = case_when(complier == 1 ~ 0, always_taker == 1 ~ 1, TRUE ~ 0),
    D1 = case_when(complier == 1 ~ 1, always_taker == 1 ~ 1, TRUE ~ 0),
    # Each unit has a single base outcome and treatment effect:
    base_Y = 1 + 0.5*X - 0.05*(X^2) + 0.025*(X^3) - 0.0025*(X^4) + 0.5*U,
    tau = rnorm(n(), LATE, 0.25) - 0.5*X + 0.1*(X^2),
    # Potential outcomes under each treatment state:
    Y_untreated = base_Y,
    Y_treated = base_Y + tau,
    # Map to potential outcomes under Z=0 and Z=1:
    Y0D0 = ifelse(D0 == 1, Y_treated, Y_untreated),
    Y1D1 = ifelse(D1 == 1, Y_treated, Y_untreated)
  ) %>%
  mutate( # Realize encouragement, treatment, and outcome:
    Z = ifelse(X >= cutoff, 1, 0),
    D = D0*(1-Z) + D1*Z,
    Y = Y0D0*(1-Z) + Y1D1*Z
  )

table(fuzzy_data$compliance_type)
...
ggplot(fuzzy_data, aes(x = X, y = Y0D0)) + 
  geom_point(color = "darkblue", alpha = 0.2) + 
  geom_smooth(color = "darkblue", method = "loess", se = F) +
  geom_point(aes(y = Y1D1), color = "darkred", alpha = 0.2) +
  geom_smooth(aes(y = Y1D1), color = "darkred", method = "loess", se = F) +
  geom_vline(xintercept = cutoff) +
  labs(title = "Potential Outcomes of Outcome", x = "X", y = "Y0D0, Y1D1") +
  theme_minimal()

ggplot(fuzzy_data[fuzzy_data$complier==1,], aes(x = X, y = Y0D0)) + 
  geom_point(color = "darkblue", alpha = 0.2) + 
  geom_smooth(color = "darkblue", method = "loess", se = F) +
  geom_point(aes(y = Y1D1), color = "darkred", alpha = 0.2) +
  geom_smooth(aes(y = Y1D1), color = "darkred", method = "loess", se = F) +
...
```{r fuzzy_realized_data}
ggplot(data = fuzzy_data[fuzzy_data$X<cutoff,], aes(x = X, y = D)) + 
  geom_point(color = "darkblue", alpha = 0.5) + 
  geom_point(data = fuzzy_data[fuzzy_data$X >= cutoff,], aes(y = D), color = "darkred", alpha = 0.5) +
  geom_vline(xintercept = cutoff) + 
  labs(title = "Realized Treatment Status", x = "X", y = "Treatment (D = 0,1)") +
  theme_minimal()

ggplot(data = fuzzy_data, aes(x = X, y = Y0D0)) + 
  geom_point(data = fuzzy_data[fuzzy_data$X<cutoff,], color = "darkblue", alpha = 0.5) + 
  geom_point(data = fuzzy_data[fuzzy_data$X >= cutoff,], aes(y = Y), color = "darkred", alpha = 0.5) +
  geom_vline(xintercept = cutoff) + 
  labs(title = "Realized Outcome", x = "X", y = "Outcome") +
  theme_minimal()
...
summary(rdrobust(fuzzy_data$Y, fuzzy_data$X_rec, c = 0, fuzzy = fuzzy_data$D))

```

## Fuzzy RDD with RDHonest

We can also estimate the fuzzy RDD using the bias-aware approach from RDHonest. The key difference from the sharp case is that we now specify the treatment variable $D$ using the `|` operator in the formula, which tells `RDHonest` to use the fuzzy (instrumental variables) estimator. As in the sharp case, `RDHonest` requires a smoothness constant $M$ — by default it estimates this from the data.

```{r fuzzy_rdhonest}
# Fuzzy RDHonest: use | to specify the endogenous treatment variable
rdh_fuzzy <- RDHonest(Y | D ~ X_rec, cutoff = 0, data = fuzzy_data)
rdh_fuzzy
