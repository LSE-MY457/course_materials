...
```{r packge_setup,message=FALSE,warning=FALSE}
...
package_check(c("dplyr", "ggplot2", "ggpubr", "ggdag", "estimatr", "AER", "sandwich"))

# dplyr for data management
# ggplot2 for graphing
# ggpubr for graphing support
...
We are interested in studying the effect of some causal variable ($D$) on some outcome variable ($Y$). As before, we will assume that $D$ is binary.
...
data <- data %>%
  mutate(
    ...
    Y1D = case_when(
      complier == 1 ~ Y0D + rnorm(N, mean = LATE, sd = 1),
      always_taker == 1 ~ Y0D, 
      never_taker == 1 ~ Y0D,
      TRUE ~ NA
    )
...
# ITT
mean(data$Y1D)  - mean(data$Y0D)

#Pr(Complier): 
mean(data$complier)

# LATE, using complier status
mean(data$Y1D[data$complier==1]) - mean(data$Y0D[data$complier==1])

# LATE as ratio of ITT and pr(complier)
(mean(data$Y1D)  - mean(data$Y0D))/mean(data$complier)
...
mean(data$complier)
mean(data$always_taker) # this is what mean(data$D[data$Z==0]) is estimating
